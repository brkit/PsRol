...
function Get-RolOrgUnitRoleGroupAssignment {
    [CmdletBinding(DefaultParameterSetName = 'ByUuid')]
    [OutputType([PsRolOrgUnitRoleGroupAssignment])]
    param (
    ...
    process {
        ...
        foreach ($uuid in $UuidsToFetch) {
            ...
            $ApiUrl = '/api/v2/organisation/{0}/assignment/rolegroup' -f [Uri]::EscapeDataString($uuid)
            $Response = Invoke-ApiClient -Uri $ApiUrl -Method 'GET'
            $ReturnObjects = foreach ($item in $Response) {
                [PsRolOrgUnitRoleGroupAssignment]::new($item)
            }
            if ($ReturnObjects) {
                Set-DefaultDisplayPropertySet -InputObject $ReturnObjects -Properties 'AssignmentId', 'OrgUnit', 'RoleGroup', 'Inherit'
                $ReturnObjects
            }
        }
    }
